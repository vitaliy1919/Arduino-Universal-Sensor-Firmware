...
int8_t _ITG3200::read(uint8_t _register)
{
    int8_t data;
...
void _ITG3200::write(uint8_t _register, uint8_t _data)
{
	Wire.begin();
	Wire.beginTransmission(gyro_address);
	Wire.write(_register);   
...
int16_t _ITG3200::read(uint8_t addressh, uint8_t addressl)
{
    int data, t_data;
...
/*           temperature sensor.                                */
double _ITG3200::getTemperature()
{
	int temp;
	double temperature;
...
void _ITG3200::getXYZ(int16_t *x,int16_t *y,int16_t *z)
{
...
void _ITG3200::getAngularVelocity(float *ax,float *ay,float *az)
{
	int16_t x,y,z;
	getXYZ(&x,&y,&z);
	*ax = x/14.375;
...
void _ITG3200::zeroCalibrate(unsigned int samples, unsigned int sampleDelayMS) 
{
